...
class AutomationAnalyzer:
    ...
    def analyze_workflow_for_automation(self, pm_workflow: str, project_title: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            prompt = (
                ...
                f"Carefully read the Product-Manager workflow for the project \"{project_title}\" below and design automation opportunities that are highly specific to THIS project.\n\n"
                "\u25c9 Generate 6–10 unique tasks, each different in scope (data-sync, QA notifications, release gates, etc.).\n"
                "\u25c9 Prefer tasks requiring multi-step orchestration or external APIs; avoid trivial single-node automations.\n"
                "For each task include: task_name, task_description, automation_benefits, integration_tools (array).\n\n"
    ...
    def _generate_n8n_workflow(self, task: Dict[str, Any], project_title: str) -> Dict[str, Any]:
        ...
        while tries < 3:
            ...
            try:
                prompt = (
                    "You are an expert n8n architect. Create a detailed n8n workflow JSON for the task "
                    f"\"{task['task_name']}\" (Project: {project_title}).\n\n"
                    "Requirements:\n"
                    ...
                    f"• Ensure every tool listed below is represented by a node in the workflow:\n  {', '.join(task.get('integration_tools', []))}\n"
                ...
                if tries >= 3:
                    ...
                    return {
                        "name": f"{task.get('task_name', 'Automation')} Workflow",
                        "nodes": [
                            {
                                "parameters": {"interval": "hour", "intervalSize": 1},
                                "id": str(uuid.uuid4()),
                            ...
                            {
                                "parameters": {"jsCode": "// Process data\nconst data = { task: '" + task.get('task_name','') + "', timestamp: new Date().toISOString() };\nreturn [{ json: data }];"},
                                "id": str(uuid.uuid4()),
                                "name": "Process Data",
